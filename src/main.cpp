...
#include "../inc/ThreadModelLivelock.h"
...
static void usage(void)
{
    ...
    std::cout << "  In the case of (CThreadModelLivelock, 2 SuperWorkers, 2 BufferItems Per BufferQueue):" << std::endl; 
    std::cout << "              ./cp ThreadModelLivelock 2 2 " << std::endl;
}
...
void handleThreadModelLivelock(int argc, char *argv[])
{
    int numSuper, numBuf;
    if (argc < 4)
    {
    ...
    numSuper = atoi(argv[2]);
    numBuf = atoi(argv[3]);
    ThreadModelLivelock tml(numSuper, numBuf);
    tml.run();
...
int main(int argc, char *argv[])
{
    ...
    else if (strcmp(argv[1], "ThreadModelLivelock") == 0)
       handleThreadModelLivelock(argc, argv);
    else
